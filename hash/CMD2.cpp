/*
 * CMD2.cpp
 *
 *   Copyright (c) 2013-2014 skdltmxn <supershop@naver.com>
 *
 * Implements CMD2 class
 *
 */

#include "stdafx.h"
#include "CMD2.h"
#include "../helper/CEndian.h"

const BYTE CMD2::s[] = {
    
};

CMD2::CMD2() : CHash(128, 128)
{
    memset(h, NULL, 48);
    memset(checksum, NULL, 16);
    updateChecksum = true;
}

CMD2::~CMD2()
{
}

void CMD2::_hashBlock(const BYTE *pbBlock)
{
    BYTE t = 0;

    // Get block
    memcpy(&h[16], pbBlock, 16);

    for (int i = 0; i < 16; ++i)
        h[32 + i] = h[16 + i] ^ h[i];

    if (updateChecksum)
    {
        t = checksum[15];
        for (int i = 0; i < 16; ++i)
        {
            t = checksum[i] ^= s[h[16 + i] ^ t];
        }
    }

    t = 0;

    // Roundings
    for (int i = 0; i < 18; ++i)
    {
        for (int j = 0; j < 48; ++j)
        {
            t = h[j] ^= s[t];
        }

        t = (t + i) & 0xFF;
    }
}

void CMD2::_padBlock(const BYTE *pbBlock, const QWORD qwLength)
{
    register int iBlockSize = (getBlockSize() >> 3);
    DWORD dwTailLength = (qwLength % iBlockSize);
    DWORD dwPadding = iBlockSize;

    PBYTE pbPaddingBlock = new (std::nothrow) BYTE[dwPadding];
    if (!pbPaddingBlock) return;

    memset(pbPaddingBlock, (dwPadding - dwTailLength), dwPadding);
    memcpy(pbPaddingBlock, pbBlock, dwTailLength);

    _hashBlock(pbPaddingBlock);
    updateChecksum = false;
    _hashBlock(checksum);

    delete []pbPaddingBlock;
}

void CMD2::_finalize(BYTE *pbHash)
{
    CEndian::encodeLittle32(pbHash, (DWORD *)h, 16);
}